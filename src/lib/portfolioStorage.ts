...
const API_KEY = '' // disabled: using local JSON mode; no API key required;
...
async saveToFile(data: PortfolioData): Promise<void> {
  ...
  async fetchFromAPI(githubUrl: string | null = null, linkedinUrl: string | null = null): Promise<PortfolioData> {
    try {
      ...
      // Network upload disabled in static mode; saving locally instead.
      // const response = await fetch(`${API_URL}/portfolio/`, { /* ... */ });
      await this.savePortfolioToLocal(data);
      // Also offer a download to user
      this.downloadPortfolioJSON(data);
if (!response.ok) {
